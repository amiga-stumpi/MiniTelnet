...
```text
MiniTelnet v0.26 by Marcel Jaehne (c)2026
```
...
- MiniTelnet always opens on a dedicated custom screen for its own ANSI palette. The screen title is `MiniTelnet Own Screen`, and the terminal writes `[MiniTelnet Own Screen]` after startup so the mode is obvious.
