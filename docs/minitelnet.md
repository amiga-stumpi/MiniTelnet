...
```text
MiniTelnet v0.28 by Marcel Jaehne (c)2026
```
...
MiniTelnet opens its own custom screen and installs an ANSI-oriented palette there. The screen depth is selected from available ChipRAM before `OpenScreen()`: up to 512 KB free ChipRAM uses 2 bitplanes, up to 1 MB uses 3 bitplanes, and more than 1 MB uses 4 bitplanes. If opening the selected depth fails, MiniTelnet falls back to the next lower depth. The palette is: pen 0 black, pen 1 white, then red, green, blue, magenta, yellow, and cyan where the screen depth allows it. The previous palette is restored on exit. Color and attribute sequences are parsed and rendered. Foreground `30..37`, background `40..47`, bold, and inverse are stored per terminal cell. On screens with at least 8 pens, MiniTelnet maps ANSI colors to Amiga pens 0..7. On lower-depth screens it falls back to a reduced palette or monochrome/inverse rendering. Unknown escape sequences are ignored instead of being printed raw.
